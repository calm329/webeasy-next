          ...
          {(appState.aiContent?.team?.list ?? []).map((person: any) => (
            <li key={person.name}>
              <img
                alt=""
                src={person.imageUrl}
                className={`aspect-[3/2] w-full rounded-2xl object-cover ${editable && "rounded border-2 border-transparent hover:border-indigo-500"}`}
              />
              <h3
                className={`mt-6 text-lg font-semibold leading-8 tracking-tight text-gray-900 ${editable && "rounded border-2 border-transparent hover:border-indigo-500"}`}
              >
                {person.name}
              </h3>
              <p
                className={`text-base leading-7 text-gray-600 ${editable && "rounded border-2 border-transparent hover:border-indigo-500"}`}
              >
                {person.role}
              </p>
